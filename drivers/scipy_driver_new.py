...
class ScipyDriverNew(ConstrainedOptimizationDriver):
    """
    ...
    def preprocess(self):
        ...
        print(self._constraints, file=outfile)
        print(self._nCon, file=outfile)
    #end
    ...
    def _eval_g(self, x, idx):
        self._evaluateFunctions(x)
        global outfile
        if idx == 0 and idx < len(self._constraintsEQ):
            #out = self._eqval[idx+1]-self._eqval[idx]
	    #out = -1E-9*self._eqval[idx]
            #out = self._eqval[idx]-self._eqval[idx]
            out = self._eqval[idx]-self._ofval[0]
        elif idx < len(self._constraintsEQ):
            out = self._eqval[idx]-self._eqval[idx-1]
        elif idx == len(self._constraintsEQ):
            out = self._gtval[idx-len(self._constraintsEQ)]
        else:
            out = self._gtval[idx]-self._gtval[idx-len(self._constraintsEQ)-1]
        #end
        print("eval_g", file=outfile)
        print(idx, out, file=outfile)
        return out
    #end
    ...
    def _eval_jac_g(self, x, idx):
        ...
        try:
            ...
            if idx == 0 and idx < len(self._constraintsEQ):
                con1 = self._constraintsEQ[idx]
                #con2 = -2*self._constraintsEQ[idx]
                #con2 = self._constraintsEQ[idx+1]
                con2 = self._objectives[0]
                f= -1.0
            elif idx < len(self._constraintsEQ):
                con1 = self._constraintsEQ[idx]
                con2 = self._constraintsEQ[idx-1]
                ...
                con1 = self._constraintsGT[idx-len(self._constraintsEQ)]
                con2 = self._constraintsGT[idx-len(self._constraintsEQ)-1]
                f = self._gtval[idx]-self._gtval[idx-1]
            #end
            ...
            if f < 0.0 or not self._asNeeded:
                if idx== len(self._constraintsEQ):
                    ...
                    if( idx == 0 and idx < len(self._constraintsEQ)):
                        #self._jac_g[:,idx] = -1E-9*con1.function.getGradient(mask) * con1.scale / self._varScales
                        #self._jac_g[:,idx] = con2.function.getGradient(mask) * con2.scale / self._varScales - (1+1E-6)*con1.function.getGradient(mask) * con1.scale / self._varScales
                        self._jac_g[:,idx] = con1.function.getGradient(mask) * con1.scale / self._varScales - con2.function.getGradient(mask) * con2.scale / self._varScales
                    else:
                        self._jac_g[:,idx] = con1.function.getGradient(mask) * con1.scale / self._varScales - con2.function.getGradient(mask) * con2.scale / self._varScales
            else:
                self._jac_g[:,idx] = 0.0
