...
class ScipyDriverInverseTest(ConstrainedOptimizationDriver):
    """
    ...
    def grad(self, x):
        ...
        try:
            ...
            for obj in self._objectives:
                self._grad_f += 2*(obj.function.getValue()-obj.target)*obj.function.getGradient(self._variableStartMask) * obj.scale * 1E-2
    ...
    def _eval_jac_g(self, x, idx):
        ...
        try:
            ...
            if f < 0.0 or not self._asNeeded:
                self._jac_g[:,idx] = con.function.getGradient(mask) * con.scale / self._varScales * 1E-2
            else:
                self._jac_g[:,idx] = 0.0
            #end
